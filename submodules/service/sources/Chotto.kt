...
class Chotto internal constructor() {

	class Builder<Context : ChottoContext, Transaction : ChottoTransaction> internal constructor() {
		...
		private var service: ChottoService<Context, Transaction>? = null
		...
		internal fun configure(assemble: suspend Builder<Context, Transaction>.() -> ChottoService<Context, Transaction>) {
			runBlocking {
				service = assemble()
			}
		}
		...
		private fun Application.configureModules() {
			...
			val entityResolverSources: MutableMap<KClass<out EntityId>, ChottoModule<*, *>> = mutableMapOf()
			...
			install(ChottoTransactionFeature(
				service = service,
				context = context
			))

			install(ChottoCommandFailureFeature)

			install(ChottoCommandRequestFeature(
				jsonCodecProvider = jsonCodecProvider
			))

			install(ChottoCommandResponseFeature(
			...
			val commandNames = mutableSetOf<ChottoCommandName>()
			val commandHandlers: MutableMap<ChottoCommandFactory<*, *, *>, ChottoCommandHandler<*, *, *>> = hashMapOf()
			for (configuration in configurations) {
			...
			for (configuration in configurations) {
				for (route in configuration.commandRoutes) {
					...
					val handler = commandHandlers[factory]
						as ChottoCommandHandler<ChottoTransaction, ChottoCommand, Any>?
					...
					route.route.handle {
						val request = call.request.pipeline.execute(call, ApplicationReceiveRequest(
							type = ChottoCommand::class,
							value = factory
						)).value as ChottoCommandRequest

						call.respond(ChottoCommandResponse(
							factory = factory,
			...
			runBlocking {
				service.onStart(context = context) // TODO we could make ChottoContext and ChottoService one thing
			}
		}
	}


	private class ProviderBasedBSONCodecRegistry<Context : ChottoContext> : CodecRegistry {
...
fun <Context : ChottoContext, Transaction : ChottoTransaction> chotto(
	assemble: suspend Chotto.Builder<Context, Transaction>.() -> ChottoService<Context, Transaction>
) {
	Chotto.Builder<Context, Transaction>().apply { configure(assemble) }.build()
}
